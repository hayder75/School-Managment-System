...
export default function AssetManagementPage() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm(t("Are you sure you want to delete this asset record?"))) return;
    try {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "Available":
        return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800"><CheckCircle2 className="w-3 h-3" /> {t("Available")}</span>;
      case "In Use":
        return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"><Building className="w-3 h-3" /> {t("In Use")}</span>;
      case "Maintenance":
        return <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800"><Wrench className="w-3 h-3" /> {t("Maintenance")}</span>;
      case "Broken":
      case "Disposed":
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 flex items-center gap-2">
            <Package className="h-7 w-7 text-primary" /> {t("School Asset & Inventory Management")}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {t("Track all school physical assets, equipment, furniture, devices, and inventory status.")}
          </p>
        </div>
        <div className="flex gap-2">
          <Button onClick={() => setReportOpen(true)} variant="outline" className="flex items-center gap-2 shadow bg-white">
            <FileText className="w-4 h-4" /> {t("Inventory Report")}
          </Button>
          ...
          <Button onClick={() => handleOpenModal()} className="flex items-center gap-2 shadow">
            <Plus className="w-4 h-4" /> {t("Add New Asset")}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">{t("Total Valuation")}</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">{t("Total Quantity")}</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{summary?.totals?.total_quantity || 0} {t("Units")}</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">{t("Available Items")}</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">{t("Under Maintenance")}</p>
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder={t("Search asset name, tag, or location...")}
            value={search}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="Electronics">{t("Electronics/Computers")}</option>
              <option value="Books">{t("Books/Library")}</option>
              <option value="Lab">{t("Lab Equipment")}</option>
        ...
        <div className="overflow-x-auto">
          <div className="overflow-x-auto -mx-px"><table className="w-full text-left text-sm">
            <thead className="bg-gray-50 border-b text-gray-500 font-medium">
              <tr>
                <th className="px-6 py-3">{t("Asset & Tag")}</th>
                ...
                <th className="px-6 py-3 text-right">{t("Total Cost")}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-gray-500">{t("Loading school assets...")}</td>
                </tr>
              ) : assets.length === 0 ? (
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-gray-500">{t("No assets found matching your query.")}</td>
                </tr>
              ) : (
                assets.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-gray-600">{item.location || t("Unassigned")}</td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {(item.status === "Broken" || item.status === "Maintenance") && (
                          <button
                            ...
                            onClick={async () => {
                              ...
                              alert(t("Maintenance ticket created."));
                            }}
                        ...
                        {item.status !== "Disposed" && (
                          <button
                            title={t("Flag as out of use / dispose")}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border relative">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {editingAsset ? t("Edit School Asset") : t("Add New School Asset")}
            </h2>

            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Asset Name")}</label>
                <input
                  type="text"
                  required
                  placeholder={t("e.g. Dell Latitude Laptop / Student Desk")}
                  value={formData.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Asset Tag / Code")}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Category")}</label>
                  <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Quantity")}</label>
                  <input
                    type="number"
                    min="1"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Unit Cost (ETB)")}</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Location / Room")}</label>
                  <input
                    type="text"
                    placeholder={t("e.g. Computer Lab A / Room 204")}
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Status")}</label>
                  <select
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Condition")}</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Useful Life (years)")}</label>
                  <input
                    type="number"
                    min="0"
                    placeholder="e.g. 5"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Assigned To (optional)")}</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Notes / Specifications")}</label>
                <textarea
                  rows="2"
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button type="submit">
                  {editingAsset ? t("Update Asset") : t("Save Asset")}
                </Button>
              </div>
            </form>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border relative">
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-1">{t("Flag as out of use / Dispose")}</h2>
        ...
        <form onSubmit={submit} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Disposal Date")}</label>
              <EthiopianDateInput value={form.date} onChange={(iso) => setForm({ ...form, date: iso })} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Method")}</label>
              <select value={form.method} onChange={(e) => setForm({ ...form, method: e.target.value })}
                className="w-full px-3 py-2 border rounded-lg text-sm bg-white">
                <option>{t("Scrapped")}</option>
                <option>{t("Sold")}</option>
                <option>{t("Donated")}</option>
                <option>{t("Lost")}</option>
                <option>{t("Other")}</option>
              </select>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">{t("Reason (required)")}</label>
            <textarea rows="2" required value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })}
              placeholder={t("e.g. Broken beyond repair / old age / lost")}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border relative max-h-[80vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-1">{t("Assignment History")}</h2>
        <p className="text-sm text-gray-500 mb-4">{asset.name} <span className="font-mono text-xs">({asset.asset_code})</span></p>
        {rows === null ? (
          <p className="py-6 text-center text-gray-500">{t("Loading…")}</p>
        ) : rows.length === 0 ? (
          <p className="py-6 text-center text-sm text-gray-500">{t("This asset has never been assigned to anyone.")}</p>
        ) : (
            ...
            {rows.map((r) => (
              <div key={r.id} className="border rounded-lg p-3 flex items-center justify-between">
                ...
                <div className="text-right text-xs text-gray-500">
                  <p>{t("from")} <EthiopianDate date={r.assigned_at} /></p>
                  <p>{r.unassigned_at ? <>{t("to")} <EthiopianDate date={r.unassigned_at} /></> : <span className="text-green-600 font-medium">{t("current holder")}</span>}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 no-print">
        <div className="bg-white rounded-2xl max-w-3xl w-full p-6 shadow-xl border relative max-h-[90vh] overflow-y-auto print-area">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{t("Fixed Asset Inventory Report")} — {selectedYear}</h2>
              <p className="text-sm text-gray-500">Mount Olive School · {t("Generated")} <EthiopianDate date={new Date()} /></p>
            </div>
            <div className="flex gap-2 no-print">
              ...
              <Button onClick={() => window.print()} className="flex items-center gap-2">
                <FileText className="w-4 h-4" /> {t("Print")}
              </Button>
            </div>
          </div>

          {loading ? (
            <p className="py-10 text-center text-gray-500">{t("Generating report…")}</p>
          ) : report && (
            <div className="space-y-6 text-sm">
              {/* Totals */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="border rounded-lg p-3"><p className="text-[11px] text-gray-500 uppercase">{t("Asset Lines")}</p><p className="text-xl font-bold">{report.totals?.items ?? 0}</p></div>
                <div className="border rounded-lg p-3"><p className="text-[11px] text-gray-500 uppercase">{t("Total Units")}</p><p className="text-xl font-bold">{report.totals?.units ?? 0}</p></div>
                <div className="border rounded-lg p-3"><p className="text-[11px] text-gray-500 uppercase">{t("Acquired")} {selectedYear}</p><p className="text-xl font-bold text-blue-600">{report.totals?.acquired_count ?? 0}</p></div>
                <div className="border rounded-lg p-3"><p className="text-[11px] text-gray-500 uppercase">{t("Disposed")} {selectedYear}</p><p className="text-xl font-bold text-orange-600">{report.disposed?.length ?? 0}</p></div>
              </div>
