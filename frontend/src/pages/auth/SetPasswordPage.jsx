...
export default function SetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setDone(true);
    } catch (err) { setError(err?.error?.message || "Set password failed"); } finally { setLoading(false); }
  }

  if (!token) return <div className="min-h-screen flex items-center justify-center"><p>Invalid invitation link</p></div>;
      ...
      <div className="w-full max-w-sm">
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full"><Lock className="h-4 w-4 mr-2" />{loading ? "Saving..." : "Set Password"}</Button>
          </form>
        )}
      </div>
    </div>
